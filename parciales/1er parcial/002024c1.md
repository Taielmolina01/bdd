...
```
SELECT  p.nombre, p.nacionalidad, e.nombre
        cir.nombre_circuito 
        COUNT(car.id_carrera) AS cantidad_carreras
FROM pilotos p
INNER JOIN equipos e ON (e.cod equipo = p.cod equipo)
INNER JOIN carreras car ON (car.cod_equipo = pi.cod_equipo AND car.nro_piloto = p.nro_piloto)
WHERE NOT EXISTS (
    SELECT 1
    FROM carreras car1 
    WHERE car1.cod_equipo = car.cod_equipo AND car1.nro_piloto = car.nro_piloto
    AND (car1.posicion NOT IN (1,2,3) OR car1.ms_mejor_vuelta <= 60000)
)
GROUP BY p.cod_equipo, p.nro_piloto, cir.id_circuito
```
