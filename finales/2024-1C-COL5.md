...
- SocioAdoraPelicula(id_socio, nombre_pelicula)

Escriba una consulta que devuelva el nombre de la/s película/s que le gustan tanto al socio mas joven como al socio mas viejo
...
Escriba una consulta que encuentre cuales son las 5 más seguidas por usuarios que tienen conocimiento en SQL.
...
Se  pide  encontrar  las  bandas  que  tengan  al  menos una canción propia con un mismo género musical que alguna canción de la banda “Eruca Sativa”.

De  todas  esas  bandas,  se  deben  devolver  únicamente  las  5  que  tengan la mayor cantidad de usuarios a los que les gusta algun tema de esa banda y también algun tema de "Eruca Sativa".
...
- Papers(id, titulo, fecha_publicacion, ciudad, categoria, contenido)
- AutoresPapers(id_paper, id_autor)
- Autores(id, nombre, apellido, nacionalidad)

```
SELECT *
FROM Papers p
    INNER JOIN AutoresPapers ap ON (p.id = ap.id_paper)
    INNER JOIN Autores a ON (a.id = ap.id_autor)
WHERE p.categroai = 'Tecnología'
```
...
$$M \geq \lceil \frac{B(Papers)}{V(A, R)} \rceil + \lceil \frac{B(AutoresPapers)}{V(id_paper, AutoresPapers)} \rceil + 1 $$
...
$$M \geq \lceil \frac{10}{100} \rceil + \lceil \frac{200}{10.000} \rceil + 1$$
...
$min(\lceil \frac{B(Papers)}{P} \rceil,\lceil \frac{B(AutoresPapers)}{P} \rceil) \leq M - 2$

$\lceil \frac{10}{999} \rceil \leq 998$
