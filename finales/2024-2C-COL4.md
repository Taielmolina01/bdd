...
Estudiantes(padron, nombre, apellido, provincia_nacimiento)

Escriba una consulta que devuelva aquellso apellidos que son tenidos por al menos un estudiante en todas las provincias de nacimiento.
...
Escriba una consulta que devuelva para cada nota de aprobacion del parcial(se aprueba con 4 o más), la cantidad de alumnos que la sacaron
...
Escriba una consulta que devuelva todas las combinaciones de restaurante, plato, principal y postre que sean apto para celíaco y que su costo total no superen los 9000 pesos.
...
SELECT *
FROM Compositores c 
    INNER JOIN obras o USING (id_compositor)
...
$$
Cost(\sigma_{año\_fallecimiento \leq 1800}(Compositores)) = B(Compositores)
$$

Según los datos del enunciado ya se que solo el 10% de la meustra falleció **antes** de 1800.

$$
n(\sigma_{año\_fallecimiento \leq 1800}(Compositores)) = 0,1 \cdot n(Compositores) = 0,1 \cdot 100.000 = 10.000
$$
...
$$
B(\sigma_{año\_fallecimiento \leq 1800}(Compositores)) = \lceil \frac{n(\sigma_{año\_fallecimiento \leq 1800}(Compositores))}{F(Compositores)} \rceil = \lceil \frac{10.000}{5} \rceil = \lceil 2.000 \rceil = 2.000
$$
...
$$
Cost(Compositores \bowtie Obras) = Cost(\sigma_{año\_fallecimiento \leq 1800}(Compositores)) + Cost(Compositores_{\sigma} \bowtie Obras)
$$
